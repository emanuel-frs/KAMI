"""Testes do router app/routers/sistema.py."""
...
def test_export_sets_last_backup_at_on_profile(client):
    ...
    body = client.get("/api/sistema/export").json()
...
def test_export_includes_known_tables_and_metadata(client):
    resp = client.get("/api/sistema/export")
...
def test_import_requires_confirmation_field(client):
    resp = client.post("/api/sistema/import", json={"tables": {"user_profile": []}})
    assert resp.status_code == 422


def test_import_rejects_wrong_confirmation_word(client):
    backup = client.get("/api/sistema/export").json()
    resp = client.post(
        "/api/sistema/import",
...
def test_import_restores_a_previous_backup(client):
    ...
    restored = client.get("/api/sistema/export").json()["tables"]
...
def test_import_leaves_data_untouched_on_bad_confirmation(client):
    ...
    logs = client.get("/api/sistema/export").json()["tables"]["action_logs"]
    assert len(logs) == 1


def test_reset_requires_confirmation_field(client):
    resp = client.post("/api/sistema/reset", json={})
    assert resp.status_code == 422


def test_reset_rejects_wrong_confirmation_word(client):
    resp = client.post("/api/sistema/reset", json={"confirmation": "sim, quero"})
    assert resp.status_code == 422
...
def test_reset_wipes_user_data(client):
    ...
    resp = client.post("/api/sistema/reset", json={"confirmation": "excluir"})
    assert resp.status_code == 200
    assert resp.json()["status"] == "ok"

    body = client.get("/api/sistema/export").json()["tables"]
    assert body["action_logs"] == []
    assert body["action_log_attributes"] == []


def test_reset_reseeds_fresh_install_defaults(client):
    client.post("/api/sistema/reset", json={"confirmation": "excluir"})
    ...
    profile_rows = client.get("/api/sistema/export").json()["tables"]["user_profile"]
    assert len(profile_rows) == 1
    assert profile_rows[0]["display_name"] == "usuário"

    income = client.get("/api/sistema/export").json()["tables"]["income_sources"]
    assert len(income) == 2
