"""Testes do router app/routers/carteira.py."""
...
def test_list_banks_returns_empty_when_none_created(client):
    ...
    (ver docstring de app/routers/carteira.py — coluna is_dinheiro removida
    via migration). DB novo começa sem nenhum banco."""
    resp = client.get("/api/carteira/banks")
    assert resp.status_code == 200
    assert resp.json() == []


def test_create_bank(client):
    resp = client.post("/api/carteira/banks", json={"nome": "Nubank", "icon_ascii": None})
    assert resp.status_code == 200
    body = resp.json()
...
def test_create_account_in_bank(client):
    bank = client.post("/api/carteira/banks", json={"nome": "Inter"}).json()
    resp = client.post(
...
def test_create_account_ignores_saldo_credito_fields_when_flags_off(client):
    bank = client.post("/api/carteira/banks", json={"nome": "Itau"}).json()
    resp = client.post(
        f"/api/carteira/banks/{bank['id']}/accounts",
        json={
            "nome": "conta simples",
...
def test_create_account_duplicate_name_in_same_bank_returns_422(client):
    bank = client.post("/api/carteira/banks", json={"nome": "Bradesco"}).json()
    client.post(
        f"/api/carteira/banks/{bank['id']}/accounts",
...
def test_create_account_in_nonexistent_bank_returns_404(client):
    resp = client.post(
        "/api/carteira/banks/banco-inexistente/accounts",
...
def test_update_account(client):
    bank = client.post("/api/carteira/banks", json={"nome": "Santander"}).json()
    conta = client.post(
        f"/api/carteira/banks/{bank['id']}/accounts",
    ...
    resp = client.put(
        f"/api/carteira/accounts/{conta['id']}",
...
def test_update_account_duplicate_name_returns_422(client):
    bank = client.post("/api/carteira/banks", json={"nome": "C6"}).json()
    conta_a = client.post(
        f"/api/carteira/banks/{bank['id']}/accounts",
        json={"nome": "conta A", "possui_saldo": True, "saldo_atual": 0},
    ).json()
    ...
    resp = client.put(
        f"/api/carteira/accounts/{conta_a['id']}",
        json={"nome": "conta B", "possui_saldo": True, "saldo_atual": 0},
    )
    assert resp.status_code == 422
...
def test_update_account_not_found_returns_404(client):
    resp = client.put(
        "/api/carteira/accounts/conta-inexistente",
...
def test_delete_last_account_of_bank_deletes_bank_too(client):
    bank = client.post("/api/carteira/banks", json={"nome": "Banco Solo"}).json()
    conta = client.post(
        f"/api/carteira/banks/{bank['id']}/accounts",
        json={"nome": "unica", "possui_saldo": True, "saldo_atual": 0},
    ).json()

    resp = client.delete(f"/api/carteira/accounts/{conta['id']}")
    assert resp.status_code == 200
    body = resp.json()
...
def test_delete_account_keeps_bank_when_others_remain(client):
    bank = client.post("/api/carteira/banks", json={"nome": "Banco Duplo"}).json()
    conta_a = client.post(
        f"/api/carteira/banks/{bank['id']}/accounts",
        json={"nome": "conta A", "possui_saldo": True, "saldo_atual": 0},
    ).json()
    ...
    resp = client.delete(f"/api/carteira/accounts/{conta_a['id']}")
    assert resp.status_code == 200
    body = resp.json()
    assert body["deleted"] is True
...
def test_delete_account_not_found_returns_404(client):
    resp = client.delete("/api/carteira/accounts/conta-inexistente")
    assert resp.status_code == 404
...
def test_summary_sums_saldo_and_fatura_across_accounts(client):
    bank = client.post("/api/carteira/banks", json={"nome": "Banco Resumo"}).json()
    client.post(
        f"/api/carteira/banks/{bank['id']}/accounts",
        json={
    ...
    resp = client.get("/api/carteira/summary")
    assert resp.status_code == 200
    body = resp.json()
...
def test_list_subscriptions(client):
    ...
    resp = client.get("/api/carteira/subscriptions")
    assert resp.status_code == 200
...
def test_update_subscription(client):
    ...
    resp = client.put(
        f"/api/carteira/subscriptions/{sub['id']}",
    ...
    body = client.get("/api/carteira/subscriptions").json()[0]
    assert body["nome"] == "Streaming Premium"


def test_update_subscription_not_found_returns_404(client):
    resp = client.put(
        "/api/carteira/subscriptions/inexistente",
...
def test_delete_subscription(client):
    ...
    resp = client.delete(f"/api/carteira/subscriptions/{sub['id']}")
    assert resp.status_code == 200
    assert resp.json()["deleted"] is True
    assert client.get("/api/carteira/subscriptions").json() == []
...
def test_delete_subscription_cascades_periods(client):
    ...
    client.get("/api/carteira/subscriptions/periods?month=2026-01")  # gera o period sob demanda

    client.delete(f"/api/carteira/subscriptions/{sub['id']}")

    resp = client.get("/api/carteira/subscriptions/periods?month=2026-01")
    assert resp.json() == []
...
def test_subscription_periods_generated_on_demand_and_idempotent(client):
    ...
    first = client.get("/api/carteira/subscriptions/periods", params={"month": "2026-03"}).json()
    assert len(first) == 1
    assert first[0]["subscription_id"] == sub["id"]
    assert first[0]["mes_ano"] == "2026-03"
    assert first[0]["paga"] is False

    second = client.get("/api/carteira/subscriptions/periods", params={"month": "2026-03"}).json()
    assert first[0]["id"] == second[0]["id"]


def test_subscription_periods_invalid_month_returns_422(client):
    resp = client.get("/api/carteira/subscriptions/periods", params={"month": "2026/03"})
    assert resp.status_code == 422
...
def test_pay_and_unpay_subscription_period(client):
    ...
    period = client.get(
        "/api/carteira/subscriptions/periods", params={"month": "2026-03"}
    ).json()[0]
    assert period["subscription_id"] == sub["id"]

    resp_pay = client.put(
        f"/api/carteira/subscriptions/periods/{period['id']}/pay",
    ...
    resp_unpay = client.put(f"/api/carteira/subscriptions/periods/{period['id']}/unpay")
...
def test_pay_period_without_valor_pago_uses_default(client):
    ...
    resp_pay = client.put(f"/api/carteira/subscriptions/periods/{period['id']}/pay", json={})
    assert resp_pay.status_code == 200
    body = resp_pay.json()
...
def test_pay_period_not_found_returns_404(client):
    resp = client.put(
        "/api/carteira/subscriptions/periods/id-inexistente/pay", json={"valor_pago": 10}
    )
    assert resp.status_code == 404


def test_unpay_period_not_found_returns_404(client):
    resp = client.put("/api/carteira/subscriptions/periods/id-inexistente/unpay")
    assert resp.status_code == 404


# ==================== compras parceladas ====================
# Sem cobertura até agora — endpoints em app/routers/carteira.py
...
def test_create_compra_parcelada_conta_sem_credito_returns_422(client):
    bank = client.post("/api/carteira/banks", json={"nome": "banco so saldo"}).json()
    conta = client.post(
        f"/api/carteira/banks/{bank['id']}/accounts",
...
def test_compra_parcelada_parcela_atual_and_quitada_computed_from_calendar(client, monkeypatch):
    ...
    monkeypatch.setattr("app.routers.carteira._current_month_str", lambda: "2026-01")
    body = client.get("/api/carteira/compras-parceladas").json()[0]
    assert body["parcela_atual"] == 1
    assert body["quitada"] is False

    monkeypatch.setattr("app.routers.carteira._current_month_str", lambda: "2026-03")
    body = client.get("/api/carteira/compras-parceladas").json()[0]
    assert body["parcela_atual"] == 3
    assert body["quitada"] is False

    monkeypatch.setattr("app.routers.carteira._current_month_str", lambda: "2026-04")
    body = client.get("/api/carteira/compras-parceladas").json()[0]
...
def test_update_compra_parcelada_sem_conta(client):
    ...
    resp = client.put(
        f"/api/carteira/compras-parceladas/{compra['id']}",
...
def test_update_compra_parcelada_not_found_returns_404(client):
    resp = client.put(
        "/api/carteira/compras-parceladas/inexistente",
...
def test_update_compra_parcelada_reconcilia_fatura_da_mesma_conta(client):
    ...
    somar por cima."""
    bank = client.post("/api/carteira/banks", json={"nome": "Nubank"}).json()
    conta = client.post(
        f"/api/carteira/banks/{bank['id']}/accounts",
    ...
    contas = client.get("/api/carteira/banks").json()[0]["accounts"]
...
def test_update_compra_parcelada_move_conta_reconcilia_ambas(client):
    ...
    contas = {a["nome"]: a for a in client.get("/api/carteira/banks").json()[0]["accounts"]}
...
def test_ajustar_parcelas_delta_positivo_avanca_parcela(client, monkeypatch):
    ...
    resp = client.put(f"/api/carteira/compras-parceladas/{compra['id']}/ajustar", json={"delta": 1})
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_ajustar_parcelas_delta_negativo_desfaz_ajuste(client, monkeypatch):
    ...
    monkeypatch.setattr("app.routers.carteira._current_month_str", lambda: "2026-01")
    client.put(f"/api/carteira/compras-parceladas/{compra['id']}/ajustar", json={"delta": 1})
...
def test_ajustar_parcelas_not_found_returns_404(client):
    resp = client.put("/api/carteira/compras-parceladas/inexistente/ajustar", json={"delta": 1})
    assert resp.status_code == 404
...
def test_ajustar_parcelas_delta_positivo_quando_ja_quitada_returns_422(client, monkeypatch):
    ...
    monkeypatch.setattr("app.routers.carteira._current_month_str", lambda: "2026-03")  # parcela_atual == 3 (último mês)
...
def test_delete_compra_parcelada_reverts_fatura_atual(client):
    ...
    resp = client.delete(f"/api/carteira/compras-parceladas/{compra['id']}")
    assert resp.status_code == 200
    assert resp.json()["deleted"] is True

    banks_after = client.get("/api/carteira/banks").json()
...
def test_delete_compra_parcelada_not_found_returns_404(client):
    resp = client.delete("/api/carteira/compras-parceladas/inexistente")
    assert resp.status_code == 404
...
def test_list_compras_parceladas_ordered_by_mes_primeira_parcela_desc(client):
    ...
    body = client.get("/api/carteira/compras-parceladas").json()
